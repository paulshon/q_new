...
def main():
    ...
    "AI 예술은 예술가와 함께 협력하여 예술의 다양성을 향상시킬 수 있다"에서 +4를 선택했으면 다음 진술문에서 +4를 선택할 수 있는 것은 1번 남았다는 의미입니다.

    또 다른 예를 들면 아래의 진술문에서 "생성형 AI로 만든 예술 작품을 구매할 의향이 있다"에서 0인 중립을 선택했으면 다음 진술문에서 0를 선택할 수 있는 것은 4번 남았다는 의미입니다.

    진술문을 체크하실 때마다 자동으로 강도의 수가 줄어들게 설정되어 있습니다.
    ...
    각 응답은 엑셀 파일로 저장됩니다. 저장된 위치는 C:/Users/Public/Documents/입니다. 엑셀 파일은 sarangred@paran.com으로 보내시면 됩니다.
    """)
    ...
    for statement in statements:
        ...
        for strength in range(-4, 5):
            if limits[strength] > 0:
                checkbox_label = f"{strength} (강한 비동의)" if strength == -4 else f"{strength} (강한 동의)" if strength == 4 else f"{strength} (중립)" if strength == 0 else str(strength)
